  ...
  return (
    <section className="container mx-auto my-20 p-5">
      <div className="divider w-3/4 mx-auto after:bg-pink-600 before:bg-pink-600 my-10">
        <h1 className="text-7xl text-center font-style-script">| Gallery |</h1>
      </div>
      <div className="w-full mx-auto grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 space-y-5 justify-center items-center gap-5 mt-20">
        {galleryImages.map((img, idx) => (
          <div key={idx} data-aos="fade-up" data-aos-duration={5000} className="mx-auto">
            <img
              src={img}
              className="w-[300px] h-[300px] rounded-lg border shadow-2xl hover:transition hover:delay-1000 hover:ease-linear hover:duration-1000 hover:scale-125"
            />
          </div>
        ))}
      </div>
